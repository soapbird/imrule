...
- [ ] Checker false passes found by review  
  **Priority:** P2  
  **Note:** (1) `setup-github-actions` and `optimize-docker` do not parse YAML flow mappings/arrays, so
  `services: {app: {privileged: true}}` passes DOPT-011 and flow-style `steps` skip pin checks.
  (2) `cli-rust` and `server-rust` cut everything after the first `#[cfg(test)]`, so a leading
  `#[cfg(test)] mod tests;` hides all production code. (3) `server-python` and `server-rust` treat
...
- [ ] Regenerate the shipped-skill digests in the release workflow  
  **Priority:** P3  
  **Note:** `src/domain/shipped_skills.rs` is regenerated by hand (`make shipped-skills`) after
  tagging a release that changed a built-in skill. Forgetting it makes that release's untouched
  copies read as modified locally in the next one. Run the script in `release.yml` (needs
  `fetch-depth: 0` for tags) and fail or open a PR when the table changes.

- [ ] Built-in checkers decode subprocess output with the locale's code page  
  **Priority:** P3  
  **Note:** `subprocess.run(..., text=True)` in `imrule-issue`, `cli`, `server`, `setup-release` and
  `setup-vscode` check.py decodes with the system encoding, so on Windows (cp1252) UTF-8 output such
  as imrule's Korean descriptions raises `UnicodeDecodeError`. `imrule-update` passes
  `encoding="utf-8"`; do the same in the others with a revision bump.
...
- [ ] `build.rs` embedding edge cases and line endings  
  ...
  **Note:** `include_str!` breaks the build on a non-UTF-8 file under `skills/`, the collector
  follows symlinks without a depth limit. (`skills/** eol=lf` in `.gitattributes` now keeps the
  Windows release from embedding CRLF.)
...
- [x] Built-in skill installed inside a user skill folder never publishes  
  **Priority:** P3  
  ...
  **Completed:** v0.5.2.0 (2026-09-18) — built-in skills are top-level folders now, so none can sit inside a user's skill folder.
