...
- **`imrule skills setup --update`**: refreshes only the built-in skills already installed — older revisions are replaced, copies under a previous name are moved — and installs nothing new. It works without a terminal, so scripts and the `imrule-update` skill can use it.
