...
전제 스킬: `setup-make`(tasks가 부르는 타깃), 언어 스킬 `cli-python`·`server-python`·`cli-rust`·`server-rust`(도구 설정 원천).
...
- 쓰지 않는 경우
  ...
  - Makefile 타깃 추가·이름 변경 → `setup-make`
...
```markdown
## setup-vscode 검사: <프로젝트> — PASS 21 · WARN 4 · FAIL 2
