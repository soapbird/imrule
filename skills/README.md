...
- `metadata.imrule-skill-version`: 스킬 파일을 하나라도 바꾸면 **1 올립니다.** `imrule skills setup`은 설치본의 값이 더 낮으면 "업데이트 가능"으로 보고 덮어쓰고, 값이 같은데 내용이 다르면 사용자가 고친 것으로 보고 `--force` 없이는 건드리지 않습니다.
