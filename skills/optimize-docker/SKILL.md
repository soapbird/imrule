...
description: "Dockerfile·.dockerignore·BuildKit/buildx·Compose·컨테이너 CI를 진단·개선해 이미지 크기, 콜드·웜 빌드 속도, 재현성, 공급망 보안, 런타임 안정성을 측정 가능하게 강화한다. 'Docker 이미지 최적화', '빌드 캐시', 'multi-stage build', 'SBOM/provenance', '취약점 스캔', 'container hardening', 'optimize Dockerfile' 요청에 사용. 태그 고정·non-root·HEALTHCHECK 같은 기본 규칙은 setup-docker."
...
# Docker 최적화 (optimize-docker)

Docker 동작을 보존하면서 프로덕션 이미지, 빌드 경로, 공급망, 런타임 설정을 측정 가능한 방식으로 개선한다.

전제 스킬: `setup-docker`. 태그 고정, non-root `USER`, exec 형식 `CMD`, `HEALTHCHECK`, `.dockerignore` 존재, 잠금 파일 설치(`--locked`), 캐시 마운트 유무는 setup-docker가 검사한다. 그쪽이 먼저 통과해야 하고, 이 스킬은 그 위의 크기·캐시·공급망·런타임 강화를 다룬다.
...
쓰지 않는 경우
- Dockerfile·compose·Makefile Docker 타깃을 처음 만들거나 기본 규칙을 맞출 때 → `setup-docker`
- 서버 코드의 헬스 엔드포인트·설정·로그 → `server`, `server-python`, `server-rust`
- 이미지를 푸시하는 워크플로 골격 → `setup-github-actions`
- 쿠버네티스 매니페스트, Helm 차트

## 모드

사용자가 모드를 말하지 않으면 **check**. 새 파일을 만드는 setup은 없다(골격은 setup-docker).
...
3. setup-docker FAIL이 남아 있으면 그 결과를 먼저 적고, 최적화 제안은 그 뒤에 둔다.
...
- `check`를 다시 돌려 setup-docker·optimize-docker에 새 FAIL이 없는지 확인한다.
...
```markdown
## optimize-docker 검사: <프로젝트> — PASS 9 · WARN 4 · FAIL 1
...
setup-docker 선행 결과: FAIL 0. 다음 단계: "최적화해 줘"라고 하면 optimize 절차로 기준선부터 측정합니다.
```
