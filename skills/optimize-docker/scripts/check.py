...
"""Docker image optimization checker (optimize-docker).
...
setup-docker (tag pinning, non-root USER, exec CMD, HEALTHCHECK, .dockerignore
...
def main() -> int:
    ...
    if not dockerfiles:
        for check_id, title in TITLES.items():
            report.skip(check_id, title, "Dockerfile 없음 — 이미지를 만든다면 setup-docker부터")
...
def _check_dockerignore(report: Report, root: Path, dockerfiles: list[Dockerfile]) -> None:
    ...
    if not ignore.is_file():
        report.skip("DOPT-009", TITLES["DOPT-009"], ".dockerignore 없음 — setup-docker DOCKER-009 먼저")
        return
