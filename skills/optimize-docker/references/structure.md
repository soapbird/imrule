# optimize-docker 구조와 패턴

기본 골격(멀티스테이지, 태그 고정, non-root, exec CMD, HEALTHCHECK, `.dockerignore`, compose.yaml, Makefile 타깃)은 `setup-docker`의 references를 따른다. 이 문서는 그 위에서 크기·캐시·공급망·런타임을 줄이고 강화하는 패턴만 담는다. 모든 변경은 SKILL.md의 측정·검증 절차를 거친다.
...
setup-docker가 요구하는 `.git`, `.env`, `target`, `.venv`, `node_modules` 외에, 저장소에 존재하고 빌드에 필요 없는 항목을 제외한다(검사: DOPT-009).
