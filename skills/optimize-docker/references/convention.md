# optimize-docker 규칙 (DOPT)

`scripts/check.py`가 확인하는 규칙(DOPT-001~017)과, 파일·측정 결과를 보고 판단하는 항목(DOPT-J01~J12)이다. 수준 `error`는 FAIL, `warn`은 WARN, `info`는 PASS에 evidence만 남긴다. 태그 고정·non-root·exec CMD·HEALTHCHECK·`.dockerignore` 존재·잠금 파일 설치·캐시 마운트 유무는 setup-docker(DOCKER-001~020)이 검사하므로 여기서 반복하지 않는다. 근거 번호는 [official-sources.md](official-sources.md)의 "우선 참고 자료" 순번이다.
