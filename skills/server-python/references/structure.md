...
- `cli/`가 필요하면 `cli-python` 규칙을 따르되 서버 코드(`users/`)를 import만 한다.
- `logging.py`라는 모듈 이름은 표준 라이브러리와 헷갈리므로 쓰지 않는다(`observability.py`).

## 2. pyproject.toml

`version`은 VERSION 파일의 4자리 값과 같게 둔다(`setup-release`).
...
`setup-make` 표준 헤더·`help`를 먼저 두고 아래 레시피를 채운다.
...
컨테이너 공통 규칙은 `setup-docker`. Python 서버는 uv 공식 멀티스테이지 패턴을 쓴다.
