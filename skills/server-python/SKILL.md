...
- 전제 스킬: `server`, `setup-make`
- 함께 쓰는 스킬: `setup-docker`, `setup-release`, `setup-github-actions`
...
- Dockerfile 공통 규칙 → `setup-docker` (이 스킬은 uv 설치 방식만 본다)
...
3. 순서:
   ...
   7. `.env.template`, Makefile(`setup-make` 표준 + `run`·`migrate`), Dockerfile(`setup-docker`)
...
```markdown
## server-python 검사: <프로젝트> — PASS 20 · WARN 8 · FAIL 1
