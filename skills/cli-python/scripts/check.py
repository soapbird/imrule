...
def main() -> int:
    ...
    if makefile is None:
        for id in ("PYCLI-024", "PYCLI-025"):
            report.skip(id, TITLES[id], "Makefile 없음 (setup-make 스킬이 검사)")
    else:
        rules = make_rules(expand_make_vars(read(makefile)))
        lint = make_recipe(rules, "lint")
        if not lint:
            report.skip("PYCLI-024", TITLES["PYCLI-024"], "lint 타깃 없음 (setup-make 스킬이 검사)")
        else:
            missing = []
        ...
        if not check:
            report.skip("PYCLI-025", TITLES["PYCLI-025"], "check 타깃 없음 (setup-make 스킬이 검사)")
        else:
