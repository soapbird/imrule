...
전제 스킬: `setup-make` — CI가 부르는 `make check`가 거기서 정의된다. 태그·VERSION 형식은 `setup-release`를 따른다.
...
쓰지 않는 경우
- Makefile 타깃 이름과 의미 → `setup-make`
- VERSION·CHANGELOG·태그 규칙 자체 → `setup-release`
- Dockerfile·compose·이미지 태그 → `setup-docker`
...
2. `Makefile`에 `check` 타깃이 있는지 본다. 없으면 `setup-make`로 먼저 만든다.
...
3. 수동 항목은 조치를 제안만 한다.
   - CI-003 개별 검사 단계를 `make check`로 합칠 때는 사라지는 단계가 Makefile `check`에 들어 있는지 먼저 확인한다. 없으면 `setup-make`로 옮긴 뒤 합친다.
...
```markdown
## setup-github-actions 검사: <프로젝트> — PASS 9 · WARN 6 · FAIL 1
