...
def main() -> int:
    ...
    if ci:
        ...
        if not calling and not _makefile_has_check(root):
            evidence += " (Makefile에 check 타깃도 없음 — setup-make 먼저)"
        report.check(
    ...
    if not (root / "VERSION").is_file():
        report.skip("CI-016", TITLES["CI-016"], "VERSION 파일 없음 (setup-release 참조)")
    elif not tagged:
