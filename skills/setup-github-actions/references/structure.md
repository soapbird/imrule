...
- 릴리스 노트는 `CHANGELOG.md`의 `## [X.Y.Z.W] - YYYY-MM-DD` 섹션을 잘라 쓴다(setup-release).
...
| 서버 컨테이너 이미지 | `release-image.yml.tmpl` | `make deploy`로 멀티 아키텍처 이미지 푸시 (setup-docker) |
