...
전제 스킬: `server` — `/healthz`·`/readyz`, 환경 변수 설정, SIGTERM 우아한 종료는 거기서 정의된다. Makefile 헤더와 `help` 규칙은 `setup-make`를 따른다.
...
- CI에서 이미지를 푸시하는 워크플로 → `setup-github-actions` (`release-image.yml.tmpl`)
- VERSION 형식 → `setup-release`
- 이미지 크기·빌드 속도·캐시·SBOM/provenance·compose 런타임 강화를 측정하며 개선 → `optimize-docker` (이 스킬이 먼저 통과한 뒤)
- 쿠버네티스 매니페스트, Helm 차트
...
```markdown
## setup-docker 검사: <프로젝트> — PASS 10 · WARN 5 · FAIL 3
