---
name: setup-make
description: "프로젝트 Makefile을 soapbird 표준(help 기본 타깃, fmt=포맷 적용·lint=읽기 전용·check=CI 게이트, .PHONY, bash 엄격 모드 헤더)으로 만들거나 준수 여부를 검사한다. 'Makefile 세팅', 'make 타깃 정리', 'Makefile 검사', 'make fmt가 검사만 해', 'check makefile conventions' 같은 요청에 사용. 빌드 도구 설정은 언어 스킬(cli-python, cli-rust 등), 이미지 구성은 setup-docker."
...
전제 스킬은 없다. 언어 스킬(`cli-python`, `server-python`, `cli-rust`, `server-rust`)과 `setup-github-actions`, `setup-docker`가 이 스킬의 타깃 규칙을 따른다.
...
- 쓰지 않는 경우
  ...
  - Dockerfile·compose 구성 → `setup-docker` (Docker 타깃 이름만 여기서 본다)
...
```markdown
## setup-make 검사: <프로젝트> — PASS 14 · WARN 3 · FAIL 2
