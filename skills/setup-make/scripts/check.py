...
def main() -> int:
    ...
    # `imrule skills setup`이 setup-make를 추천하는 기준(Makefile·Cargo.toml·pyproject.toml)과 맞춘다.
    ...
    if makefile_path is None:
        if not has_project:
            for check_id, title in TITLES.items():
                report.skip(check_id, title, "Makefile도 Cargo.toml/pyproject.toml도 없음 — setup-make 대상 아님")
            return report.emit(fmt)
    ...
    if uses_docker:
        ...
        report.check("MK-035", TITLES["MK-035"], not missing_docker, severity="warn",
                     ...
                     fix="setup-docker 스킬 기준으로 docker-build / docker-push / deploy 정리")
    else:
