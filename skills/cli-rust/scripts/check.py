...
def main() -> int:
    ...
    if (root / "VERSION").is_file():
        ...
        skip("RSCLI-018", "VERSION 파일 없음 (setup-release 스킬이 검사)")
    ...
    if not makefile.is_file():
        skip("RSCLI-025", "Makefile 없음 (setup-make 스킬이 검사)")
        skip("RSCLI-026", "Makefile 없음 (setup-make 스킬이 검사)")
    else:
        ...
        if lint:
            ...
            skip("RSCLI-025", "lint 타깃 없음 (setup-make 스킬이 검사)")
        ...
        if check:
            ...
            skip("RSCLI-026", "check 타깃 없음 (setup-make 스킬이 검사)")
