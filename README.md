...
cd imrule
make build
make install              # 바이너리를 ~/.local/bin/imrule로 복사
sudo make install-system  # 또는 /usr/local/bin/imrule로 복사
```
...
```bash
make install PREFIX=/opt/imrule    # /opt/imrule/bin/imrule에 설치(기본값 ~/.local)
```

### 제거

```bash
make uninstall                          # ~/.local/bin/imrule 제거
sudo make uninstall PREFIX=/usr/local   # /usr/local/bin/imrule 제거
```
...
`imrule apply`가 생성한 자산을 제거합니다. 규칙 파일, `.bak` 백업, 스킬 복사본, 서브에이전트 파일, gitignore 관리 블록, MCP 항목이 대상이며, `--agents` 없이 전체를 정리할 때는 `.imrule/manifest.json`과 `.imrule/cache.json`도 지웁니다. `clear`는 `agents` 설정과 무관하게 기본적으로 모든 에이전트를 대상으로 하며, `--agents`로 범위를 제한할 수 있습니다.
...
> **안전성**: 규칙 파일은 `<!-- Generated by ImRule -->`로 시작할 때만 삭제합니다. 스킬 복사본과 서브에이전트 파일은 마지막 `apply`가 매니페스트에 기록했거나 지금 `.imrule/`에서 만들어지는 내용과 바이트 단위로 같을 때만 지웁니다. 에이전트 디렉터리에 직접 넣은 스킬과 서브에이전트는 남고, 디렉터리는 비었을 때만 삭제합니다. 지우거나 다시 쓰는 경로가 심볼릭 링크를 따라가도 프로젝트 안에 있는지 디스크에서 확인하므로, 저장소에 커밋된 매니페스트나 링크가 프로젝트 밖을 가리켜도 그곳은 건드리지 않습니다.
...
```bash
make                  # 타깃 목록 출력
make setup            # rustfmt·clippy 설치와 의존성 받기
make check            # CI 게이트: 포맷 검사 + 린트 + 테스트(파일 수정 없음)
make fmt              # 포맷 적용
make fmt-check        # 포맷 검사
make lint             # cargo clippy(경고를 에러로)
