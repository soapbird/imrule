...
make shipped-skills  # scripts/shipped-skills.py → src/domain/shipped_skills.rs (after tagging a release that changed a built-in skill)
...
### Domain (`src/domain/`)
- `builtin_skills.rs` — Built-in skill catalog built from embedded files and ordered by name-prefix group (`cli`, `server`, `setup`, `optimize`, `imrule`), `RENAMED_BUILTIN_SKILLS` (old path → current name, so `python/cli` still resolves and an old copy moves to `cli-python`), `ProjectSignals` → `recommend_builtin_skills` detection rules, and `BuiltinSkillState` (not installed / up to date / outdated revision / modified locally).
...
- `shipped_skills.rs` — Generated by `scripts/shipped-skills.py` (`make shipped-skills`, from the release tags) table of every built-in skill revision a release installed, with per-file FNV-1a digests; `BuiltinSkillState::compare` counts an older copy as `Outdated` only when it matches one exactly, otherwise `Modified`.
...
- `skills_setup_use_case.rs` — Plans built-in skills against the project (recommended + install state + a copy left under a previous path) and installs the selected ones, refreshing outdated revisions, moving old-path copies to their current name, and skipping locally modified skills unless asked to overwrite. `installed_paths` is what `setup --update` refreshes.
...
- **Adding or changing a built-in skill**: Edit files under `skills/<name>/` following `skills/README.md` (one flat folder per skill, named with its kind prefix: `cli-rust`, `setup-make`; frontmatter `name` = folder name, bump `metadata.imrule-skill-version` on every content change, keep the `check.py` helper block identical). `build.rs` embeds the tree; update the `SHIPPED` list in `skills_setup_contract.rs` and, for a new skill, its detection rule in `recommend_builtin_skills`. After tagging a release that changed a built-in skill, run `make shipped-skills` and commit `src/domain/shipped_skills.rs`; otherwise copies that release installed read as modified locally in the next one. When renaming one, add the old path to `RENAMED_BUILTIN_SKILLS` and to `RENAMED` in `skills/imrule-update/scripts/check.py`.
