...
- `imrule revert` — restores agent config files from backups and removes generated content.

This is a native Rust project (edition 2021, MSRV 1.80). It was rewritten from a prior TypeScript/npm runtime; no JS/TS artifacts remain in the source tree.
...
### Domain (`src/domain/`)
- `agent.rs` — Compile-time `const` array of 32 `AgentDefinition`s (identifier, name, output paths, MCP keys, capabilities). This is the single source of truth for the agent registry.
- `config.rs` — Config structs: `LoadedConfig`, `AgentConfig`, `McpConfig`, `GitignoreConfig`, `SkillsConfig`, `SubagentsConfig`, `SubagentFrontmatter`.
...
- `mcp.rs` — MCP capability filtering, remote→stdio transformation, and merge/overwrite logic.
...
- `ports.rs` — Five trait boundaries: `ConfigPort`, `FileSystemPort`, `GitignorePort`, `McpPort`, `AgentWriterPort`.
- `apply_use_case.rs` — Orchestrates reading `.imrule/`, concatenating rules, writing agent files, merging MCP configs, and updating gitignore.
- `init_use_case.rs` — Scaffolds `.imrule/` or `~/.config/imrule/` non-destructively.
- `revert_use_case.rs` — Restores from `.bak` files, removes generated files, and clears gitignore managed blocks.
...
- `config_loader.rs` — `TomlConfigLoader`: resolves `imrule.toml` (local → global fallback), parses all TOML sections, supports legacy `subagents` table alias.
...
- `cli.rs` — Clap derive definitions for `Cli`, `ApplyArgs`, `InitArgs`, `RevertArgs`.
...
|---|---|
| `agent_layer_contract.rs` | Full agent registry (32 agents): identifiers, names, output paths, MCP keys, capabilities. `DefaultAgentWriter` idempotency and custom output path overrides. |
...
| `cli_release_contract.rs` | Binary-level tests via `assert_cmd::Command`: `--version`, `--help`, real `apply`/`revert` round-trip, `--dry-run`, `init` idempotency, `init --global` with `XDG_CONFIG_HOME`. |
| `config_fs_rules_contract.rs` | `concatenate_rules` format, `read_markdown_files` ordering/skipping, `GitignoreUpdater` managed block behavior, `TomlConfigLoader` parsing, parent-directory creation on write. |
...
| `skills_subagents_apply_revert_contract.rs` | Skills discovery/warnings/copy/gitignore paths; subagent frontmatter parse/validate/load; Claude/Cursor/Codex/Copilot file builders; apply path collection; revert-level file operations. |
...
- Backup files (`.bak`) are created in the same directory as the original file. The revert command restores from `.bak` or deletes generated files if no backup exists.
...
- **Changing file write behavior**: Modify `infrastructure/agent_writer.rs` or `infrastructure/file_system.rs`, then update `config_fs_rules_contract.rs` or `skills_subagents_apply_revert_contract.rs`.
- **Changing config parsing**: Modify `infrastructure/config_loader.rs` and update `config_fs_rules_contract.rs`.
- **Changing MCP logic**: Modify `domain/mcp.rs` or `infrastructure/mcp_storage.rs` and update `mcp_paths_settings_contract.rs`.