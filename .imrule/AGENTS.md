...
- `ports.rs` — Trait boundaries: `ConfigPort`, `ConfigWritePort`, `FileSystemPort`, `GitignorePort`, `GitTrackingPort`, `ManifestPort`, `McpPort`, `AgentWriterPort`, `SkillFetcherPort`.
...
- `clear_use_case.rs` — Removes all assets generated by `apply` across every supported agent, including the ones the current config no longer describes (found via the manifest).
...
- `cli.rs` — Clap derive definitions for `Cli`, `ApplyArgs`, `ClearArgs`, `InitArgs`, `McpArgs`, `SkillsArgs`, `SkillsSetupArgs`.
...
| `skills_setup_contract.rs` | Built-in catalog grouping and name resolution, detection rules and project signal collection, install/update/modified/dry-run semantics, picker filtering/toggling/rendering, every shipped skill complete (name = path, revision, references, `check.py` helper block identical to `skills/README.md`), and `skills setup --list`/names/`--yes` through the binary. |
| `skills_subagents_apply_clear_contract.rs` | Skills discovery/warnings/copy/gitignore paths; skill source registry recording and `skills update` statuses; subagent frontmatter parse/validate/load; Claude/Cursor/Codex/Copilot file builders; apply path collection; filesystem-level operations used by `clear`. |
