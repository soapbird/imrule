...
- `shipped_skills.rs` — Generated (`make shipped-skills`, from the release tags) table of every built-in skill revision a release installed, with per-file FNV-1a digests; `BuiltinSkillState::compare` counts an older copy as `Outdated` only when it matches one exactly, otherwise `Modified`.
