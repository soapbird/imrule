...
- **Adding or changing a built-in skill**: Edit files under `skills/<name>/` following `skills/README.md` (one flat folder per skill, named with its kind prefix: `cli-rust`, `setup-make`; frontmatter `name` = folder name, bump `metadata.imrule-skill-version` on every content change, keep the `check.py` helper block identical). `build.rs` embeds the tree; update the `SHIPPED` list in `skills_setup_contract.rs` and, for a new skill, its detection rule in `recommend_builtin_skills`. When renaming one, add the old path to `RENAMED_BUILTIN_SKILLS` and to `RENAMED` in `skills/imrule-update/scripts/check.py`.
