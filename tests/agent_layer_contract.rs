...
#[test]
fn droid_is_an_accepted_alias_for_factory() {
    // Factory's CLI ships as `droid`, so both names must select one adapter.
    assert_eq!(canonical_agent_identifier("droid"), "factory");
    assert_eq!(canonical_agent_identifier("factory"), "factory");
    // Unknown names pass through so callers still report them as unknown.
    assert_eq!(canonical_agent_identifier("nonexistent"), "nonexistent");

    let aliased = find_agent("droid").expect("droid resolves");
    assert_eq!(aliased, find_agent("factory").expect("factory resolves"));
    assert_eq!(aliased.identifier, "factory");
    assert!(find_agent("nonexistent").is_none());

    // Aliases stay out of the canonical help listing.
    assert!(!get_agent_identifiers_for_cli_help().contains("droid"));
}
