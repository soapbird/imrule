...
test-e2e: build
	...
	$(BINARY) init --project-root $(TMP)/init-local
	@grep -q "Imrule Configuration File" $(TMP)/init-local/.ruler/ruler.toml && echo "[PASS] init did not overwrite existing files" || (echo "[FAIL] init overwrote existing files" && exit 1)
